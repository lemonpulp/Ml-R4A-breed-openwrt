...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='NingMeng'' package/lean/default-settings/files/zzz-default-settings

# 版本号里显示一个自己的名字（ababwnq build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i 's/OpenWrt /编译时间 $(TZ=UTC-8 date "+%Y.%m.%d") @ 柠檬果肉 /g' package/lean/default-settings/files/zzz-default-settings
...
sed -i 's/ssid=OpenWrt/ssid=NingMeng-XiaoMi/g' package/kernel/mac80211/files/lib/wifi/mac80211.sh
